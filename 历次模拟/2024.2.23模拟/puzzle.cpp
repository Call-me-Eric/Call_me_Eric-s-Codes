...
bool SPFA(int S,int T){
    queue<int> que;
    for(int i = 1;i <= totpoint;i++)dis[i] = INF,book[i] = 0;
    que.push(T);book[T] = true;dis[T] = 0;
    while(!que.empty()){
        int u = que.front();que.pop();book[u] = false;
        for(int i = head[u];i;i = e[i].nexte){
            int v = e[i].to;
            if((e[i ^ 1].cap > e[i ^ 1].flow) && (dis[v] > dis[u] + e[i ^ 1].cost)){
...
int dfs(int u,int flow,int T){
    ...
    for(int i = head[u];i;i = e[i].nexte){
        int v = e[i].to;
        if(!vis[v] && e[i].cap > e[i].flow && dis[u] - e[i].cost == dis[v]){
            int tmp = dfs(v,min(flow,e[i].cap - e[i].flow),T);
            if(tmp){
                mincost += tmp * e[i].cost;res += tmp;
...
pair<int,int> MCMF(int S,int T){
    ...
    while(SPFA(S,T)){
        vis[T] = 1;
        while(vis[T]){
            for(int i = 1;i <= totpoint;i++)vis[i] = 0;
            maxflow += dfs(S,INF,T);
        }
    }
