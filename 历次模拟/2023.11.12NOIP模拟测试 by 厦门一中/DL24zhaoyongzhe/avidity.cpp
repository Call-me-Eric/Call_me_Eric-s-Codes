...
//	for(auto i : tmp)printf("%d %d %d\n",i.first.first,i.first.second,i.second);
	for(int test = 1;test <= m;test++){
		...
		for(auto i : tmp){
			...
			x = upper_bound(vec[x].begin(),vec[x].end(),r) - lower_bound(vec[x].begin(),vec[x].end(),l);
			y = upper_bound(vec[y].begin(),vec[y].end(),r) - lower_bound(vec[y].begin(),vec[y].end(),l);
			z = upper_bound(vec[z].begin(),vec[z].end(),r) - lower_bound(vec[z].begin(),vec[z].end(),l);
			lst += x * y * z;
		}
		printf("%lld\n",lst);
	}
