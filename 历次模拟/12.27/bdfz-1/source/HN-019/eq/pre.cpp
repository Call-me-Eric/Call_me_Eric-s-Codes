...
int main()
{
	...
	while (q--)
	{
		...
		if (ds)
		{
			swap(a[rt],a[ds]),swap(b[rt],b[ds]);
			for (int i=1;i<=n;++i)
				if (i!=ds&&a[i][ds]&&a[ds][ds])
				{
					d=(long long)(a[i][ds])*fast_pow(a[ds][ds],mod-2)%mod;
					for (int j=1;j<=n+1;++j)
						if (a[ds][j])
							Adder2(a[i][j],-(long long)(a[ds][j])*d%mod);
					for (int j=1;j<=n;++j)
						if (b[ds][j])
							Adder2(b[i][j],-(long long)(b[ds][j])*d%mod);
				}
		}
		for (int i=1;i<=n;++i)
