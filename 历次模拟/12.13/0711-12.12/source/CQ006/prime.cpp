...
vector<string> ans, f = {"0", "1", "4", "6", "8", "9"}, g;
...
void init() {
    ...
    } ans = f, ans.erase(ans.begin());
    for(int i = 1; i < 9; i++) {
        for(int j = 0; j < 6; j++) for(auto l : f) {
            s = l + e[j]; bool f1 = 0;
            for(int k = 4; k < 26 and !f1; k++) f1 = check(s, k);
            if(!f1) g.push_back(s);
        } swap(f, g), g.clear();
        for(auto l : f) if(l.back() != '0') ans.push_back(l);
    } 
    s = f[0], s.push_back('1'), ans.push_back(s);
    // for(auto l : ans) {
    //     s = l, reverse(s.begin(), s.end());
    //     cout << s << '\n';
    // }
}
int work(int x) {
