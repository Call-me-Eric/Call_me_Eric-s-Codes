...
node query(int u,int v){
    node ans = node();int col = c[u];
    while(top[u] != top[v]){
        if(dep[top[u]] < dep[top[v]])swap(u,v);
        ans = mergenode(ans,tree.query(dfn[top[u]],dfn[u],col));
        u = fa[top[u]];
    }
    if(dfn[u] > dfn[v])swap(u,v);
    ans = mergenode(ans,tree.query(dfn[u],dfn[v],col));
    return ans;
}
void main(){
    ...
    for(int i = 1;i <= q;i++){
        ...
        if(opt[0] == 'C'){
            if(opt[1] == 'C'){
                tree.update(dfn[u],0,head[c[u]]);
                c[u] = v;
                tree.update(dfn[u],w[u],head[c[u]]);
            }
            else{
                w[u] = v;
                tree.update(dfn[u],w[u],head[c[u]]);
