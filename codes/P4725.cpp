...
namespace polyinv{
    ...
    vector<int> solve(vector<int> arr,int mod,int n = -1){
        NTT::init(mod);
        a = arr;if(n == -1)n = a.size();
        int mxn = n * 5;
...
namespace polyln{
    ...
    void getln(vi &f,vi &g,int n){
        Derive(f,A,n);B = polyinv::solve(f,mod,n);

        len = 1;L = 0;while(len < (n << 1))len <<= 1,L++;
        for(int i = 0;i < len;i++){rev[i] = (rev[i >> 1] >> 1) | ((i & 1) << (L - 1));}

        A.resize(len + 10);B.resize(len + 10);
        NTT::NTT(A,len,1);NTT::NTT(B,len,1);
        for(int i = 0;i < len;i++)A[i] = A[i] * B[i] % mod;
        NTT::NTT(A,len,-1);int inv = qpow(len,mod - 2);
		for(int i = 0;i < len;i++){A[i] = A[i] * inv % mod;}
        Integrate(A,g,n);
    }
    vi solve(vi arr,int md){
        NTT::init(md);
        a = arr;int n = a.size(), mxn = n * 5;rev.resize(mxn);
        a.resize(mxn);b.resize(mxn);A.resize(mxn);B.resize(mxn);
        int m = 1;for(;m <= n;m <<= 1);getln(a,b,m);
        b.resize(n);return b;
    }
};
int n;
vector<int> vec;
void main(){
    n = read();for(int i = 1;i <= n;i++)vec.push_back(read());
    auto ans = polyln::solve(vec,998244353);
