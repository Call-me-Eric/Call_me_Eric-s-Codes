...
int getans(){
    Matrix tmp = tree.query(dfn[1],end[1]).sum;
    return max(tmp.a[0][0],tmp.a[1][0]);
}
void main(){
    n = read(); m = read();char ch[20];scanf("%s",ch + 1);
    int sum = 0;
    ...
    dfs1(1,0);dfs2(1,1);tree.build(1,n,1); int u, v, a, b;
    // printf("%lld\n",getans());
    for(int i = 1;i <= m;i++){
        u = read(); a = read(); v = read(); b = read();
        if((fa[u] == v || fa[v] == u) && (a == b && b == 0)){puts("-1");continue;}
        int tu = val[u], tv = val[v];
        update(u,val[u] + (1 - 2 * a) * 0x3f3f3f3f);
        update(v,val[v] + (1 - 2 * b) * 0x3f3f3f3f);
        int ans = getans();
        ans -= (!a + !b) * 0x3f3f3f3f;
        printf("%lld\n",sum - ans);
        update(u,tu);update(v,tv);
    }
    return;
}
};
